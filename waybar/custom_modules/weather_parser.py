...
def main():
    ...
    text = f"{icon}  {weather['temp_C']} °C"
    tooltip = "\n".join(
        [
            condition,
            f"Temperature: {weather['temp_C']} °C",
            f"Feels like {weather['FeelsLikeC']} °C",
            f"High of {data['weather'][0]['maxtempC']} °C / Low of {data['weather'][0]['mintempC']} °C",
            f"{weather['humidity']}% Humidity",
            f"{weather['pressure']} HPa Pressure",
            f"{weather['uvIndex']} UV Index",
            f"{weather['precipMM']} mm. Precipitation",
            f"{weather['visibility']} km. Visibility",
            f"{weather['windspeedKmph']} km/h Wind Speed, Going {weather['winddir16Point']} / {weather['winddirDegree']}°",
